...
class Update_offer extends StatefulWidget {
  const Update_offer({super.key});

  @override
  State<Update_offer> createState() => _Update_offerState();
}
...
class _Update_offerState extends State<Update_offer> {
  bool check=false;
          ...
          children: [
            Form(
              ...
              child: Column(children: <Widget>[
                        ...
                        child: ElevatedButton(
                            ...
                            onPressed: () {
                              showDialog(context: context, builder: (context){
                                return AlertDialog(
                                  backgroundColor: Colors.white70,
                                  title: Text("SELECT THE TIME TO SUBTRACT",textAlign: TextAlign.center,),
                                  content:SizedBox(
                                      height: 180,
                                      child: _Integer()
                                  ),
                                  actions: [
                                    ElevatedButton(
                                        ...
                                        onPressed: () async {
                                          check=true;
                                          DateTime dateTime = DateTime.parse(controller_form);
                                          dateTime=dateTime.subtract(Duration(hours: second_hours,minutes: second_minutes));
                                          controller_form =dateTime.toString();
                                          _fromController.text = DateFormat.jmv().format(dateTime);
                                          Navigator.pop(context);
                                        },
                ...
                Text("ENTER THE EXPECTED TIME TO FINISH AFTER (UPDATED):",style: TextStyle(fontSize: 19),textAlign: TextAlign.center,),
                SizedBox(
                  height: 15,
                ),
                _IntegerExample(),
        ...
        onPressed: () async {
          if(validate.currentState!.validate()) {
            ...
            try {
              ...
              bool mass=false;
              // if(check){
              //   controller_form = controller_form.replaceRange(
              //       controller_form.length - 14,
              //       controller_form.length - 13,
              //       '');
              //   controller_form = controller_form.replaceAll("  ", " ");
              // }
              DateTime dateTime = DateTime.parse(controller_form);
              sql mysql=sql();
              if(Hours==0&&minutes==0){}
                ...
                if(kind.text=="Multi"){
                  ...
                  costdouble=place.text=="Ps6"?((Hours+(minutes/60))*25):((Hours+(minutes/60))*20);
                  if(place.text=="Ps6")
                    costdouble=costdouble%5<3.2?costdouble-(costdouble%5):costdouble+(5-(costdouble%5));
                  else
                    costdouble=costdouble%5<3.2?costdouble-(costdouble%5):costdouble+(5-(costdouble%5));
                  cost_demo=costdouble.toInt();
                }else{
                  place.text=="Ps6"?mass=false:mass =true;
                  costdouble=(place.text=="Ps6")?((Hours+(minutes/60))*20):((Hours+(minutes/60))*15);
                  if(place.text=="Ps6")
                    costdouble=costdouble%5<3.2?costdouble-(costdouble%5):costdouble+(5-(costdouble%5));
                  else
                    costdouble=costdouble%5<2.2?costdouble-(costdouble%5):costdouble+(5-(costdouble%5));
                  cost_demo=costdouble.toInt();
                }
                costst=cost_demo.toString();
              }
              controller_to=controller_to.isEmpty?"Still Running":controller_to;
              await mysql.update("offers", {
                "kind": kind.text,
                "place": place.text,
                "frm": controller_form,
                "too": controller_to,
            ...
            Expanded(
              ...
              child: Center(
                child: NumberPicker(
                  ...
                  itemWidth: 30,
                  itemCount: 5,
                  selectedTextStyle: TextStyle(color: Colors.deepPurple, fontSize: 22),
                  infiniteLoop: true,
                  decoration: BoxDecoration(
                    borderRadius: BorderRadius.circular(7),
                    border: Border.all(color: Colors.black26),
